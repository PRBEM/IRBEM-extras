...
public:
    Main(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
        ...
        //
        ASSERT(7==nlhs && 18==nrhs, "Wrong number of input/output.");

        //
        // (xin [np, nt], yin [np, nt], zin [np, nt], [year, doy, hour, min, sec] (5, nt), ioptparmod [1 or 10, nt], external, internal, ionoR, ds, dx, dy, n_phi, n_theta, shouldKeepContour, isCartesian, M_threads, shouldUseInterpolatedField)
        //
        M = mxGetM(prhs[0]);
        N = mxGetN(prhs[3]);
        ...
        interpolationOrder = round( mxGetScalar(prhs[17]) );
