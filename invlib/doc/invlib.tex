...
Whereas the TEM-1 method required a constrained fit, the Vampola
method requires only a numerical integral:

\begin{eqnarray}
f_{\rm uni}(\alpha) &=& f_0 \sin^n\alpha, \\
f_{\rm iso} &\approx& \sum_j H_j f_0 \sin^n \alpha_j , \\ 
\verb|uniflux| &=& \frac{f_{\rm iso}}{\sum_{j} H_j f_0 \sin^n \alpha_j}.
\end{eqnarray}
