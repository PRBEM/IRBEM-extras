...
int ana_spec_inv(const double *y, const double *dy, 
                 ...
                 double *flux, double *dlogflux, double *support_data);
\end{verbatim}
...
\subitem \verb|(support_data[ASI_MAX_NQ+1)*k]| $\ell$ for the spectral function with bit mask $2^{k-1}$ (dimensionless).
\subitem \verb|(support_data[ASI_MAX_NQ+1)*k+m]| fit parameter $q^{(k)}_m$ for the spectral function with bit mask $2^{k-1}$. (usually dimensionless, see function)
\end{itemize}
...
\item \verb|ana_spec_inv| operates on {\it counts} not {\it count rate}b|
...
\item  \verb|ASI_MAX_NQ| is 10 at the moment, providing for up to 10 free parameters in some future fit function. It may change, but that's unlikely.
	\verb|ASI_MAX_NQ| is also defined in \verb|specinv.h| for C users.
\end{enumerate}
...
\verb|NE|. \verb|support_data| is advanced by $(\verb|ASI_MAX_NQ|+1)\times(\verb|ASI_MAX_POW2|+1)$,
