...
of the log flux explained by each vector.

Therefore, for any time, we can write:
\begin{eqnarray}
\vec{x}(t) &=& \bar{\vec{x}} + \dbul{V}\vec{q}(t), \\
...
The $\vec{q}$'s are uncorrelated, nominally Gaussian variables with zero mean and variance
given by the corresponding entries in $\vec{d}$:
\begin{equation}
p(\vec{q}) = \frac{1}{(2\pi)^{N_q/2}||\vec{d}||}\exp\left[-\frac{1}{2}\vec{q}^T\dbul{D}^{-1}\vec{q}\right].
\end{equation}
(The underlying assumption of this prior model is that each flux $f(E_j)$ is
distributed as a log-normal distribution over time).
...
constrain the solution $\vec{q}$). Because of the presence of prior
...
error covariance for $\vec{q}$ and $\vec{x}$ (log flux) are given by:
\begin{eqnarray}
\cov(q_m,q_{m'}) &=& \left(
\begin{array}{ccc}
~ & \vdots & ~ \\
...
= 
\sum_{m,m'}V_{jm}\cov(q_m,q_{m'})V_{j'm'}.
\end{eqnarray}
