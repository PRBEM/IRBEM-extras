...
\begin{itemize}
\item{11/18/2010} \verb|ana_spec_inv| now uses the geometric mean of the energy grid to normalize initial fit parameters
\item{11/17/2010} Function descriptions expanded and new section added for Python interface and test suite (S. Morley)
...
\exp(q_1 - q_2\ln E_{\rm break} - (E-E_{\rm break})/E_0) & E > E_{\rm break}.
\end{array}
\right.
\end{equation}
where we have chosen this parameterization so that there is no
...
\begin{figure}
\center{
\includegraphics[width=40pc]{pc_model.pdf}
}
...
\begin{figure}
\center{
\includegraphics[width=40pc]{pc_spec_inv_example.pdf}
}
...
This function takes a set of counts \verb|y| (per accumulation interval), estimated relative error
...
instrument response function, where for \verb|ana_spec_inv| the response 
function must include the aggregation time. For each call to \verb|ana_spec_inv|, a
...
\item \verb|b| (input) \verb|b[NY*t+i]| $=$ expected background counts in channel \verb|i| for case \verb|t|. \\
Best fit when \verb|y[i]|~$\approx$~\verb|b[NY*t+i]|~+~$\sum_j$~\verb|H[NY*j+i]*dt[t]*flux[NE*t+j]|.
...
This section describes the angular inversion functions for
...
\section{High-level language interfaces to INVLIB}
\subsection{Python}
An interface to the C inversion library has been written in Python to allow rapid
scripting and testing of the library. The package is compatible with Python 
versions $\geq$2.6, including Py3k.

To install the Python package, both Python and NumPy must be installed first. If
MatPlotLib is installed additional visualization tools will be available. In the 
directory containing the inversion library, type:\\
\begin{verbatim}
python setup.py install --home=<dir>
\end{verbatim}
where the \verb|--home| option defines a non-standard installation directory. To install
in the default location (by omitting the flag) may require administrative privileges.
To access the interface from within Python, import the package \verb|pyinvlib|.

The unit-testing suite included with this interface, ``invlib\_test.py'', can be run
either directly from the checked-out SVN repository or from the install directory. It
includes tests for a number of methods within the interface, as well as basic regression
tests for the underlying C library. Currently this represents the most comprehensive
test suite for the C inversion library and it is recommended that the library be tested 
using this suite with any development of the code.

As usual, the documentation for \verb|pyinvlib| is contained in the source code in the'
form of docstrings. From a Python environment, this can be accessed using the Python 
built-in \verb|help()| function (within the iPython interpreter simply appending a 
\verb|?| will do). e.g.,
\begin{verbatim}
...
The package defines two new classes for the user, \verb|SpecInv| and \verb|AngInv|, for
spectral inversion and angular inversion, respectively. (Note: these are sub-classed from
the \verb|InvBase| class, which is not designed to be instantiated and provides jointly 
inherited behavior).

For comprehensive and up to date documentation on the Python interface, either read the
help from the Python environment or use a package like ``epydoc'' to automatically generate
comprehensive documentation in either HTML or PDF form.
