...
switch(inst_info.ET_TYPE),
    case 'TBL',        
        % min/max forces plateau extrapolation in E
        inst_info.internal.R = @(inst_info,E,theta,phi)interpn(inst_info.E_GRID,inst_info.TH_GRID,inst_info.R,min(max(E,inst_info.E_GRID(1)),inst_info.E_GRID(end)),theta,'linear',0); % set to zero outside grid
    otherwise
        error('ET_TYPE %s not defined yet',inst_info.ET_TYPE);
end
