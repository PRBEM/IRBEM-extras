...
tmp_chans['ARB']['R'] = 3 * np.maximum(0, np.arctan((EI-2)*10)) * \
    np.exp(-3*(THI/15)**2)*(1+rfl.cosd(PHI)**2)
...
[EI,THI] = np.meshgrid(tmp_chans['ARBC']['E_GRID'],
                       tmp_chans['ARBC']['TH_GRID'], indexing='ij')
tmp_chans['ARBC']['R'] = 3 * np.maximum(0, np.arctan((EI-2)*10)) * \
    np.exp(-3*(THI/15)**2)
...
[THI, PHI] = np.meshgrid(tmp_chans['ESEP']['TH_GRID'],
                         tmp_chans['ESEP']['PH_GRID'], indexing='ij')
...
for info in [inst_info, jsonname, jsonnamebig, h5name, h5namebig]:
    ...
    for v in info['CHANNEL_NAMES']:
        ...
        if hasattr(resp,'ar'):
            ...
            grids = rfl.broadcast_grids(resp.E_GRID, resp.TH_GRID,
                                        resp.PH_GRID)
            tmp = resp.R(*grids)
            ...
            raise Exception('Could not figure out how to compute G: %s,%s' % \
                            (v, resp.RESP_TYPE))
